...
const ProductAllergensCheckboxes: React.FC<Props> = ({
  ...
  return <div className="my-[20px] mx-[20px]">
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        return <label key={allergen.id} className={cn("flex items-center gap-2 p-2 border rounded-md cursor-pointer hover:bg-muted/50 transition-colors", checked ? "border-primary bg-muted/50" : "border-input")}>
              <Checkbox checked={checked} onCheckedChange={checked => handleChange(allergen.id, checked as boolean)} />
