...
export function OnlineMenuAddToCartButtonSettings({
  value,
  onChange
}: OnlineMenuAddToCartButtonSettingsProps) {
  ...
  return <div className="flex flex-col gap-4 border rounded-md p-4 my-4 bg-black/0">
      ...
      <div className="flex gap-4 flex-wrap">
        {ICONS.map(opt => <label key={opt.value} className="flex flex-col items-center cursor-pointer">
            <input type="radio" name="buttonIcon" value={opt.value} checked={icon === opt.value} onChange={handleIconChange} className="mb-1 accent-primary" />
