...
import { Settings, LayoutList, Image, Folder, FileText, Text, Star, StickyNote } from "lucide-react";
...
const PrintLayoutEditorSidebar: React.FC<SidebarProps> = ({
  activeTab,
  setActiveTab
    ...
    <nav>
      <ul className="flex md:flex-col gap-1">
        {SECTIONS.map(section => <li key={section.key}>
            <button className={`w-full flex items-center gap-2 px-3 py-2 rounded transition
                ${activeTab === section.key ? "bg-primary/90 text-primary-foreground font-bold shadow" : "hover:bg-muted text-muted-foreground"}
              `} onClick={() => setActiveTab(section.key)} aria-current={activeTab === section.key ? "page" : undefined} type="button">
              {section.icon}
