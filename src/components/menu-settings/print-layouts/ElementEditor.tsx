...
interface ElementEditorProps {
  title: string;
  // Supporta sia la vecchia prop 'element' che la nuova 'elementConfig'
...
export const ElementEditor: React.FC<ElementEditorProps> = ({
  ...
  // Usa element o elementConfig, dando priorità a elementConfig se entrambi sono forniti
  const config = elementConfig || element;
  
  if (!config) {
    console.error('ElementEditor: Né elementConfig né element sono stati forniti');
    return null;
  }
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  <Label htmlFor={`${title}-lineHeight`}>
                    Interlinea: {roundToOneDecimal(config.lineHeight || 1.5)}
                  </Label>
                  <Slider
                    id={`${title}-lineHeight`}
                    min={1}
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="space-y-1">
                    <Label htmlFor={`${title}-margin-top`} className="text-xs">
                      Superiore: {config.margin.top}mm
                    </Label>
                    <Input
                      id={`${title}-margin-top`}
                      type="number"
                  ...
                  <div className="space-y-1">
                    <Label htmlFor={`${title}-margin-bottom`} className="text-xs">
                      Inferiore: {config.margin.bottom}mm
                    </Label>
                    <Input
                  ...
                  <div className="space-y-1">
                    <Label htmlFor={`${title}-margin-left`} className="text-xs">
                      Sinistra: {config.margin.left}mm
                    </Label>
                    <Input
