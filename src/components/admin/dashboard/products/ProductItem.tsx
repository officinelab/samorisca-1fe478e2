...
interface ProductItemProps {
  ...
  canMoveUp: boolean;    // Rimangono nei props, ma non più usati
  canMoveDown: boolean;  // Rimangono nei props, ma non più usati
  onClick: () => void;
  onEdit: () => void;
  onDelete: () => void;
  onMoveUp: () => void;
...
const ProductItem: React.FC<ProductItemProps> = ({
  product,
  isSelected,
  // isReordering,      // Non necessario per il click/frecce ora
  // canMoveUp,
  // canMoveDown,
  onClick,
  onEdit,
  ...
  // Nuove classi per la nuova UI
  const baseClass = [
    "grid",
    "grid-cols-[48px,72px,1fr,48px]",
    "items-center",
    "border",
    "rounded-xl",
    "px-2",
    ...
    <div className={baseClass} onClick={onClick}>
      ...
      <div className="flex flex-col flex-1 min-w-0 gap-1">
        ...
        <div className="flex flex-wrap items-center gap-2 mt-1">
          {/* Prezzo standard e suffisso prezzo */}
          ...
          {product.has_price_suffix && product.price_suffix && (
            <span className="text-xs text-gray-500 ml-1">{product.price_suffix}</span>
          )}
          {/* Allergeni */}
