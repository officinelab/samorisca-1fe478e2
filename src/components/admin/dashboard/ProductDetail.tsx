...
const ProductDetail: React.FC<ProductDetailProps> = ({
  ...
  const prices = [product.price_variant_1_value !== null && product.price_variant_1_name ? {
    label: product.price_variant_1_name,
    value: Number(product.price_variant_1_value).toFixed(2)
  } : null, product.price_variant_2_value !== null && product.price_variant_2_name ? {
    label: product.price_variant_2_name,
      ...
      <Card className="mb-4">
        <div className="flex flex-col md:flex-row gap-6 p-6">
          ...
          <div className="flex-shrink-0 flex justify-center items-center">
            <CardImage src={product.image_url || "/placeholder.svg"} alt={product.title} square className="w-32 h-32 rounded-md overflow-hidden border shadow-sm bg-gray-50" />
          </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              ...
              {prices.length > 0 && <div>
                  ...
                  <div className="flex flex-col gap-1 mt-1">
                    {prices.map((p, idx) => <div key={idx} className="flex items-center gap-2">
      ...
      {allergens.length > 0 && <Card className="mb-4">
          <CardContent>
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {allergens.map(all => <div key={all.id} className="bg-gray-100 rounded-full px-3 py-1 flex items-center text-xs">
                  {all.icon_url && <img src={all.icon_url} alt={all.title} className="w-4 h-4 mr-1" />}
