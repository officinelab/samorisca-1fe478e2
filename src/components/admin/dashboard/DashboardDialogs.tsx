...
const DashboardDialogs: React.FC<DashboardDialogsProps> = ({
    ...
    return <>
        ...
        if (!open) {
          ...
          <DialogContent className="sm:max-w-md">
            ...
            <CategoryForm category={editingCategory} onSave={memoizedCategoryFormSave} onCancel={memoizedCategoryFormCancel} />
          </DialogContent>
        </Dialog>

        {/* Product Form Dialog */}
        {productDialogOpen && <Dialog open={productDialogOpen} onOpenChange={open => {
        if (!open) {
          handleProductFormCancel();
        }
      }}>
            <DialogContent className="sm:max-w-4xl max-h-[90vh] overflow-hidden">
              ...
              <div className="overflow-y-auto max-h-[calc(90vh-8rem)]">
                {productDialogOpen && <ProductForm key={memoizedEditingProduct?.id || 'new'} product={memoizedEditingProduct} categoryId={selectedCategoryId || undefined} onSave={handleProductFormSave} // <-- Prop onSave viene passata sempre
  ...
  return <>
      ...
      {/* Product Form Sheet */}
      {productDialogOpen && <Sheet open={productDialogOpen} onOpenChange={open => {
      if (!open) {
        handleProductFormCancel();
      }
    }}>
          <SheetContent className="sm:max-w-4xl overflow-hidden">
            ...
            <div className="pt-4 overflow-y-auto max-h-[calc(100vh-8rem)] py-0">
              {productDialogOpen && <ProductForm key={memoizedEditingProduct?.id || 'new'} product={memoizedEditingProduct} categoryId={selectedCategoryId || undefined} onSave={handleProductFormSave} // <-- Prop onSave viene passata sempre
          onCancel={handleProductFormCancel} />}
            </div>
          </SheetContent>
