...
-- Fix per la funzione increment_tokens - non aggiorna il limite del mese corrente
...
BEGIN
  ...
  -- Crea record per il mese corrente se non esiste, usando il limite dalle impostazioni
  INSERT INTO public.translation_tokens (month, tokens_used, tokens_limit, purchased_tokens_total, purchased_tokens_used)
  VALUES (current_month, 0, get_monthly_tokens_limit(), 0, 0)
  ON CONFLICT (month) DO NOTHING;
  
  -- Ottieni stato attuale dei token SENZA aggiornare il limite esistente
  SELECT 
    COALESCE(tt.tokens_used, 0), 
    COALESCE(tt.tokens_limit, 300), -- Usa il limite già esistente nel record
  ...
  -- Aggiorna il database SENZA modificare il limite esistente
